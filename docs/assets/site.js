...
(function (global) {
  ...
  /* The address of a page, as inc/boot.php's static_url() writes it: the
     browsing pages are files, the reading pages keep their query strings. */
  function u(page, params) {
    ...
    switch (page) {
      ...
      case 'plan.php': return 'plan-' + (params.id || '') + '.html';
      case 'library.php': {
        var r = params.religion || '';
        var s = params.section || '';
        var p = parseInt(params.p || 1, 10) || 1;
        if (!r) return 'library.html';
        var name = 'library-' + r;
        if (s) {
          name += '-' + s;
          if (p > 1) name += '-' + p;
      ...
      case 'work.php': {
        var w = params.work || '';
        var wp = parseInt(params.p || 1, 10) || 1;
        return 'work-' + w + (wp > 1 ? '-' + wp : '') + '.html';
      }
      default: {
        ...
        return q.length ? page + '?' + q.join('&') : page;
      }
    }
  }
  ...
  var DATA_BUNDLE = 'data/bundle.bin';
  var DATA_INDEX = 'data/bundle.index.json.gz';

  function bundleIndex(indexUrl) {
    return memo('bi:' + indexUrl, function () {
      return bodyText(indexUrl).then(function (t) { return JSON.parse(t); });
    });
  }

  /* The bytes of one part, decompressed. */
  function partOf(bundleUrl, indexUrl, name) {
    return memo('p:' + bundleUrl + '#' + name, function () {
      return bundleIndex(indexUrl).then(function (index) {
        ...
        return fetch(bundleUrl, {
          ...
          /* No range support: the whole file came back; take the part. */
          if (!r.ok) throw new Error(r.status + ' ' + bundleUrl);
  ...
  function partText(bundleUrl, indexUrl, name) {
    return partOf(bundleUrl, indexUrl, name).then(function (buf) {
      return new TextDecoder('utf-8').decode(buf);
    });
  }

  function bundleJson(bundleUrl, indexUrl, name) {
    return partText(bundleUrl, indexUrl, name).then(function (t) { return JSON.parse(t); });
  }
  ...
  function loadCatalog() {
    if (!state.catalog) {
      state.catalog = bundleJson(DATA_BUNDLE, DATA_INDEX, 'catalog');
    }
  ...
  function canonMap(workId) {
    ...
    if (!state.canon[file]) state.canon[file] = bundleJson(DATA_BUNDLE, DATA_INDEX, file.replace('.json', ''));
    return state.canon[file];
  }

  function canonBook(workId) {
    return bundleJson(DATA_BUNDLE, DATA_INDEX, 'links/books').then(function (books) {
      var b = books[workId];
  ...
  function linksRead(rel) {
    return bundleJson(DATA_BUNDLE, DATA_INDEX, 'links/' + rel.replace(/\.json$/, ''));
  }
  ...
  function linkBooks() {
    return bundleJson(DATA_BUNDLE, DATA_INDEX, 'links/books').catch(function () { return {}; });
  }

  /* ---------- plans, Strong's, word study, the daily verse ---------- */

  function plans() {
    if (!state.plans) state.plans = bundleJson(DATA_BUNDLE, DATA_INDEX, 'plans');
  ...
  function strongs() {
    if (!state.strongs) {
      state.strongs = bundleJson(DATA_BUNDLE, DATA_INDEX, 'strongs').then(function (d) {
  ...
  function wordstudy() {
    if (!state.wordstudy) {
      state.wordstudy = bundleJson(DATA_BUNDLE, DATA_INDEX, 'wordstudy').then(function (d) {
  ...
  function wordindex() {
    if (!state.wordindex) state.wordindex = bundleJson(DATA_BUNDLE, DATA_INDEX, 'wordindex');
    return state.wordindex;
  }

  function daily() {
    if (!state.daily) state.daily = bundleJson(DATA_BUNDLE, DATA_INDEX, 'daily');
    return state.daily;
  }
  ...
  function align(bookKey, chapterN) {
    return bundleJson(DATA_BUNDLE, DATA_INDEX, 'align/' + bookKey + '/' + chapterN)
      .catch(function () { return null; });
  }
  ...
  function kjvBooks() {
    if (!state.kjvBooks) state.kjvBooks = bundleJson(DATA_BUNDLE, DATA_INDEX, 'kjv_books');
...
(function () {
  ...
  function loadBooks(cb) {
    ...
    (window.AA
      ? AA.part('data/bundle.bin', 'data/bundle.index.json.gz', 'kjv_books')
...
(function () {
  ...
  /* The search index lives in the data bundle; each shard is fetched by
     its own byte range, so a query transfers one shard. */
  var BUNDLE = 'data/bundle.bin';
  var INDEX = 'data/bundle.index.json.gz';
  var PER_PAGE = 25;
  ...
  function manifest() {
    if (!manifestP) manifestP = AA.part(BUNDLE, INDEX, 'search/manifest').then(function (t) { return JSON.parse(t); });
    return manifestP;
  }

  function units() {
    if (!unitsP) {
      unitsP = AA.part(BUNDLE, INDEX, 'search/units').then(function (t) {
        return t.split('\n');
      });
    }
    return unitsP;
  ...
  function findRef() {
    ...
    return AA.part(BUNDLE, INDEX, 'kjv_search').then(function (t) { var idx = JSON.parse(t);
  ...
  function wordSearch() {
    ...
    Promise.all([manifest(), units()]).then(function (both) {
      ...
      return Promise.all(stems.map(function (s) {
        ...
        if (!cache[shard]) cache[shard] = AA.part(BUNDLE, INDEX, 'search/w' + shard).then(function (t) { return JSON.parse(t); });
