...
/* ===== home.js ===== */
/* The Abrahamic Library — the home page.
 *
 * The figures, the verse for the day, each tradition's counts and the six
 * suggestions for where to start all come from the catalog and the daily
 * list. This fills the shell in with them.
 */
(function () {
  'use strict';
  ...
  function fill(name, text) {
    var el = document.querySelector('[data-figure="' + name + '"]');
    if (el) el.textContent = text;
  }

  AA.catalog().then(function (cat) {
    var t = cat.totals || {};
    fill('works', AA.num(t.works));
    fill('chapters', AA.num(t.chapters));
    fill('verses', AA.num(t.verses));
    fill('words', AA.num(t.words));

    cat.religions.forEach(function (r) {
      var el = document.querySelector('[data-faith-count="' + r.id + '"]');
      if (el) {
        el.textContent = AA.num(r.totals.works) + ' works \u00b7 ' +
          AA.num(r.totals.words) + ' words';
      }
    });

    /* Where to start: the texts each tradition opens with. */
    var starts = [
      ['jps-genesis', 'The Torah opens', '1'],
      ['webu-jhn', 'The fourth gospel', '1'],
      ['quran-pickthall', 'The Opening', '1'],
      ['webu-gen', 'In the beginning', '1'],
      ['sef-pirkei-avot', 'The Fathers', '1'],
      ['webu-psa', 'The Psalms', '23']
    ];
    Promise.all(starts.map(function (s) {
      return AA.work(s[0]).then(function (m) { return { id: s[0], why: s[1], c: s[2], meta: m }; });
    })).then(function (items) {
      document.querySelector('[data-starts]').innerHTML = items.filter(function (x) {
        return x.meta;
      }).map(function (x) {
        var translator = (x.meta.edition && x.meta.edition.translator) ||
          AA.religionName(x.meta.religion);
        return '<a class="work-card" href="' +
          AA.e(AA.u('read.php', { work: x.id, c: x.c })) + '">' +
          '<b>' + AA.e(x.meta.title) + (x.c !== '1' ? ' ' + AA.e(x.c) : '') + '</b>' +
          '<small>' + AA.e(x.why) + ' \u00b7 ' + AA.e(translator) + '</small></a>';
      }).join('');
    });
  });

  /* The verse for the day. It is chosen by the date, so everyone who opens
     the archive on the same day meets the same verse. */
  AA.verseOfTheDay().then(function (vod) {
    if (!vod) { root.closest('section').remove(); return; }
    root.setAttribute('data-r', vod.religion || '');
    document.querySelector('[data-daily-text]').textContent = vod.text;
    var ref = document.querySelector('[data-daily-ref]');
    ref.href = AA.u('read.php', { work: vod.work, c: vod.c }) +
      '#v' + String(vod.v).replace(/[^0-9a-z]/gi, '');
    ref.textContent = vod.ref;
    var ed = document.querySelector('[data-daily-edition]');
    if (vod.edition) ed.textContent = ' \u00b7 ' + vod.edition;
  });
})();

/* ===== staticpages.js ===== */
/* The Abrahamic Library — the plans list, and the about page's figures.
 *
 * Both pages have one part the PHP site computed on the server: the list of
 * plans, grouped by religion, and the tally of what the modernizer changed
 * and the table of rights statements. This fills both in from data/plans.json
 * and the catalog.
 */
(function () {
  'use strict';
  ...
  /* ---------- the plans list ---------- */
  var holder = document.querySelector('[data-plans-list]');
  if (holder) {
    AA.plans().then(function (plans) {
      var labels = {
        judaism: 'Judaism', christianity: 'Christianity',
        islam: 'Islam', '': 'Across the scriptures'
      };
      var groups = {};
      var order = [];
      ...
      holder.innerHTML = order.map(function (key) {
        var cards = groups[key].map(function (p) {
          return '<a class="work-card plan-card" href="' +
            AA.e(AA.u('plan.php', { id: p.id })) + '"' +
            (p.religion ? ' data-r="' + AA.e(p.religion) + '"' : '') +
            ' data-plan-card="' + AA.e(p.id) + '" data-days="' + p.days.length + '">' +
            '<b>' + AA.e(p.name) + '</b>' +
            '<span class="plan-blurb">' + AA.e(p.blurb) + '</span>' +
            '<small>' + p.days.length + ' days &middot; ' +
              p.days.reduce(function (n, d) { return n + d.readings.length; }, 0) +
              ' readings</small>' +
            '<span class="plan-progress" data-plan-progress hidden>' +
              '<span class="bar"><span></span></span><em></em></span>' +
            '</a>';
        }).join('');
        return '<div class="plan-group" data-religion-group="' + AA.e(key) + '">' +
          '<h2 class="plan-group-heading">' + AA.e(labels[key] || key) + '</h2>' +
          '<div class="grid-works">' + cards + '</div></div>';
      }).join('');

      /* The progress bars are painted by assets/plan.js, which ran when the
         page loaded; tell it the cards are here. */
      document.dispatchEvent(new CustomEvent('aa:plans'));
    });
  }

  /* ---------- the about page ---------- */
  var changed = document.querySelector('[data-about-changed]');
  if (changed) {
    /* The reports are not shipped — they are a build-time audit — so the
       sentence states what the archive did in words rather than figures. */
    changed.hidden = true;
  }

  var rights = document.querySelector('[data-rights-body]');
  if (rights) {
    AA.catalog().then(function (cat) {
      var sources = {};
      cat.religions.forEach(function (r) {
        r.sections.forEach(function (s) {
          s.works.forEach(function (w) {
            if (w.borrowed_from) return;
            var key = (w.rights && w.rights.statement) || 'Public domain.';
            if (!sources[key]) {
              sources[key] = { statement: key, religions: {}, works: 0, examples: [] };
            }
            var src = sources[key];
            src.works++;
            src.religions[r.name] = true;
            if (src.examples.length < 3 && w.edition && w.edition.translation) {
              src.examples.push(w.edition.translation);
            }
          });
        });
      });
      var list = Object.keys(sources).map(function (k) { return sources[k]; });
      list.sort(function (a, b) { return b.works - a.works; });

      var rows = list.slice(0, 25).map(function (s) {
        return '<tr style="border-bottom:1px solid var(--line-soft);vertical-align:top">' +
          '<td style="padding:.6rem .5rem .6rem 0"><strong>' +
            AA.e(Object.keys(s.religions).join(', ')) + '</strong>' +
            (s.examples.length
              ? '<br><span class="muted small">e.g. ' +
                AA.e(s.examples.slice(0, 2).join('; ')) + '</span>' : '') +
          '</td>' +
          '<td style="padding:.6rem .5rem">' + AA.num(s.works) + '</td>' +
          '<td style="padding:.6rem .5rem" class="muted">' + AA.e(s.statement) + '</td>' +
        '</tr>';
      }).join('');
      rights.innerHTML = rows;

      var rest = list.slice(25);
      var restEl = document.querySelector('[data-rights-rest]');
      if (rest.length && restEl) {
        restEl.textContent = '\u2026 and ' + AA.num(rest.length) +
          ' further rights statements covering ' +
          AA.num(rest.reduce(function (n, s) { return n + s.works; }, 0)) +
          ' works, each named on that work\u2019s own page. Every one is ' +
